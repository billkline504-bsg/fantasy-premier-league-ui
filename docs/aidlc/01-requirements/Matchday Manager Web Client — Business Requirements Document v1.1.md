...
## Business Requirements Document — Version 1.1
...
1. **No authentication, registration, or invitation-acceptance screens exist in the mock-up.** The application is depicted entirely from an already-authenticated session. A production client needs login, (if self-registration is in scope) registration, password reset, and a flow for accepting a league invitation (`BR-027`–`BR-029`) — none of which this BRD can specify from the mock-up alone. **Confirmed as deliberately out of scope for this pass** (per the v1.1 planning decision to proceed to `02-architecture`/`04-user-stories` on the 17 mock-up screens only) — this remains open and will need its own BRD revision before it can be built, since the backend API surface for it (`/api/v1/auth/*`, `/api/v1/invitations/{token}/accept`) already exists and is otherwise unused by this client.
2. **No league-creation or league-administration-setup screens exist.** League Administrator actions like creating a league, sending invitations, or configuring the settings later shown read-only on the History screen's "config snapshot" (squad size, positional minimums, draft timer, points-per-result, invitation expiration) have no corresponding input screen in the mock-up. **Confirmed as deliberately out of scope for this pass**, for the same reason as item 1.
