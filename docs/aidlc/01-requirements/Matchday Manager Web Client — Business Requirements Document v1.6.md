...
- **Administrator-transfer** — v1.5 designs League Creation, Settings, Invitations, and Members (§8.21–8.24, resolving §12 item 2), but deliberately not a transfer-administrator control: no such endpoint exists anywhere in the backend's API despite `leaveLeague`'s own error response referencing "transferring administration" (see new §12 item 13). This document does not invent a UI for a capability the backend doesn't implement.
...
- Visual design polish beyond what the mock-up already establishes (the mock-up's dark/light palette, typography, and component shapes are treated as an accepted starting point, not as final art direction) — this now extends to the auth/onboarding screens added in v1.4 and the league-administration screens added in v1.5, both of which reuse the same established visual language even though the mock-up never depicted either (see the notes at the top of §8.17 and §8.21).
- Any business rule not already reflected in a mock-up screen or, as of v1.4/v1.5, already designed in §8.17–8.24. If the backend BRD defines a rule with no corresponding screen and no source material to design one from (administrator transfer), this document does not invent a screen for it — it lists the omission in §12 instead.
...
**v1.5 additionally reads**: the same OpenAPI spec's `createLeague`, `updateLeague`, `getLeagueConfiguration`/`updateLeagueConfiguration`, `createSeason`, `getSeasonConfiguration`/`updateSeasonConfiguration`, `createInvitation`/`listInvitations`/`revokeInvitation`, and `leaveLeague` operations, and the backend BRD's `BR-023`–`BR-026`, `BR-283`, `BR-290`–`BR-297` — §8.21–8.24 have no mock-up screen either (confirmed absent, §12 item 2), so this version designs them the same way v1.4 designed §8.17–8.20: directly from the already-implemented backend contract.
...
A prominent panel names the team currently on the clock, the manager, the pick number, and a live countdown (UIR-013) to the pick timer's expiration (`BR-057`), plus a note that an administrator may extend it (`BR-058`). *(Amended in v1.6 — while the Draft is `Paused`, the live countdown is replaced entirely per UIR-216; see §12 item 11's resolution.)*
...
A "Draft" action button exists per available-player row, but must only be usable by the FantasyTeam currently on the clock (UIR-101) — every other manager views the same pool read-only while another team picks. *(Amended in v1.6 — the action is additionally disabled for everyone, including the on-clock team, while the Draft is `Paused`; see UIR-215.)*
...
**UIR-214–218 (added in v1.6) resolve §12 item 11** — the Draft-Paused state, previously an open gap with no source material. Unlike the auth/onboarding and league-administration gaps this document has resolved in prior versions (v1.4/v1.5), the backend's own BRD (the usual source these designs reconcile against) contains **zero business rules of any kind about pausing or resuming a Draft** — no rationale for why an Administrator would pause one, no stated behavior for the pick timer during or after a pause, and no documented outcome for a pick attempted while paused. `Draft.status` including `Paused`, and the `pause`/`resume` operations, exist in the OpenAPI spec with nothing beyond a one-line summary each. Given that near-total silence, these UIRs are deliberately conservative: where the contract doesn't say what happens, this design chooses the interpretation least likely to mislead or produce an inconsistent result, and says so plainly rather than presenting an assumption as a confirmed rule.

## UIR-214 — Paused-State Explanation Banner
Whenever the active Draft's status is `Paused`, the Draft Board leads with a rule-explanation banner (the same pattern already required by `UIR-029`/`DEC-UI-002`) stating plainly that the Draft is paused, that no picks can be made until a League Administrator resumes it, and that this applies to every viewer, not only the team currently on the clock.

## UIR-215 — Picks Blocked While Paused, for Everyone
While the Draft's status is `Paused`, the "Draft" action (UIR-103) is disabled for every manager, including whichever team was on the clock when the pause began — not merely left restricted to that team as it already is during normal play. This is enforced client-side regardless of what the backend itself would do with a pick submitted mid-pause, since the API does not document that outcome either way (see the note above UIR-214) — the safer assumption is that a pause means no picks happen, not that the backend is trusted to reject one correctly.

## UIR-216 — Timer Display Replaced, Not Left Running or Frozen at a Guessed Value
While the Draft's status is `Paused`, the on-the-clock panel (UIR-101) does not show a live countdown at all — ticking, frozen, or otherwise — replacing it with a static "Paused" indicator instead. Neither whether the timer keeps running server-side during a pause, nor what value it resumes from, is documented anywhere; showing any specific countdown value would imply a confidence about that behavior this document cannot support.

## UIR-217 — Pause/Resume Control, League-Administrator-Only
The Draft Board provides a League Administrator with a Pause action while the Draft is `InProgress`, and a Resume action while it is `Paused` (`BR-025`'s "manage draft timing") — visible only to that League's Administrator, per the same access pattern already established for Audit Log/Score Corrections/League Settings.

## UIR-218 — Pause/Resume State Reflected Without a Manual Refresh
A Draft being paused or resumed becomes visible to every other viewer within the same few-second polling window the Draft Board already refreshes on (UIR-101's live-timer mechanism), not only to the Administrator who triggered it and not only after a manual page reload.

## 8.10 Makeup Picks & Timeouts

**Note (added in v1.6):** Makeup Picks & Timeouts (UIR-110–117) is not yet built (§12 item 11's scoping decision keeps this pass limited to Draft Board, which already exists). Whenever this screen is eventually built, it should apply the same Paused-state treatment UIR-214–218 establish for Draft Board — the live timer (UIR-115) replaced the same way (UIR-216), the same explanation banner (UIR-214), and picks/actions blocked the same way (UIR-215) — rather than rediscovering the same open questions independently.
...
**§8.21–8.24 (added in v1.5) have no mock-up screen behind them either** — confirmed absent in §12 item 2 since v1.0, the same way §8.17–8.20 had none. They're designed directly from the backend's already-implemented `createLeague`, `updateLeague`, `getLeagueConfiguration`/`updateLeagueConfiguration`, `createSeason`, `getSeasonConfiguration`/`updateSeasonConfiguration`, `createInvitation`/`listInvitations`/`revokeInvitation`, and `leaveLeague` operations (see §3), reusing this client's own established conventions (the locked-field pattern already used by Season Predictions/History, the recoverable-inline-error pattern from Profile/Register, card/table/form styling) rather than mock-up interpretation.

## 8.21 League Creation

## UIR-191 — League Creation Form
A form collects a League name and description (`BR-023`) and, since a League with no Season can't do anything else in it, the details for its first Season in the same step: an EPL season identifier and start date (`BR-032`), with a single Create League action.

## UIR-192 — No EPL-Season-Identifier Picker
The EPL season identifier field is free text (e.g. "2026/27"), not a dropdown of valid values — there is no endpoint to discover which EPL seasons exist (the same gap already documented for EPL/Draft Board, API Consumption Specification §2.3d), so this is the first *editable* instance of a gap previously only encountered in read-only contexts.

## UIR-193 — Creator Becomes Administrator
Immediately after creation, a confirmation states plainly that the creator is now this League's sole Administrator (`BR-024`), since nothing else about a freshly created, memberless League would otherwise make that clear.

## UIR-194 — Reachable With Zero Leagues, or Alongside Existing Ones
League Creation is reachable both from the "no leagues yet" empty state (UIR-177) and from the league switcher (UIR-002) at any time — creating an additional League never requires leaving or losing access to any League the user already belongs to.

## UIR-195 — Post-Creation Redirect
After a successful creation, the user lands on the new League's Dashboard, with it now selected as the active League in the switcher.

## 8.22 League Settings

## UIR-196 — League Details Editing
A form edits the League's name, description, and status (Active/Archived) (`BR-025`), pre-filled with the League's current values.

## UIR-197 — Two-Tier Configuration Editor
Configuration is edited in two clearly separated sections mirroring the backend's own model (`BR-292`, `BR-294`): **League Defaults**, which only affects Seasons created afterward or fields not yet locked on the current Season, and **This Season's Configuration**, which affects only the active Season and never the League's stored defaults or any other Season.

## UIR-198 — Every Configurable Parameter Is Editable, Not Just What History Shows
Every parameter in the backend's configurable-parameter table (`BR-291`) is editable here — squad size, weekly roster size, positional minimums, draft pick timer (independently per draft type), secondary draft selections per team, secondary draft scheduling offset, Gameweek roster lock offset, points per result, invitation expiration, and replacement selection cap — not only the six History's read-only config snapshot happens to surface (UIR-093).

## UIR-199 — Locked Fields Are Visually Read-Only
A Season Configuration field already past its `BR-291` "Locks At" point for the active Season renders using the same locked-field treatment already established for locked/historical data elsewhere (UIR-027) — not merely disabled with no explanation — so an administrator understands *why* a field can't be changed here, not just that it can't.

## UIR-200 — Save Confirms Which Values Actually Changed
On a successful save, a confirmation states which fields actually changed (old → new), consistent with the same audit-visible framing Audit Log/Score Corrections already give this exact kind of change (`BR-295`) elsewhere in the product.

## UIR-201 — Settings Is League-Scoped and Administrator-Only
Like Audit Log and Score Corrections, League Settings reflects and edits only the active League; switching Leagues re-scopes it entirely, and it is reachable only by that League's Administrator.

## 8.23 Invitations

## UIR-202 — Invitation Form
A form collects a destination (email address or phone number) and a channel (Email or Text/SMS) (`BR-028`), with a single Send Invitation action.

## UIR-203 — Invitation Expiration Stated at Send Time
Directly beside the form, the screen states the League's current invitation-expiration window (`BR-029`, e.g. "Invitations expire after 7 days") so an administrator knows the window before sending, not only after.

## UIR-204 — No Link or Token Is Ever Shown
Once sent, the screen confirms delivery ("Invitation sent to member@example.com") but never displays the invitation link or token itself — the backend's `Invitation` record carries no such field, delivery happens entirely through the chosen channel, and there is no manual-share fallback to design around that absence.

## UIR-205 — Pending/Expired/Revoked Invitations List
A table lists every invitation ever issued for the League — destination, channel, status (Pending/Accepted/Expired/Revoked), and expiration date — reverse-chronological by creation.

## UIR-206 — Revoke Action, Pending Invitations Only
A Revoke action is available only on a Pending invitation; an already-Accepted, Expired, or Revoked one shows no action, since revoking only ever makes sense before it's been used or has already lapsed on its own.

## UIR-207 — Invitations Is League-Scoped and Administrator-Only
Like League Settings, Invitations reflects and edits only the active League and is reachable only by that League's Administrator.

## 8.24 League Members

## UIR-208 — Member List, Viewable by Every Member
A table lists every member of the active League — username, an Administrator role badge where applicable, join date, and status (Active/Left) — viewable by any member of the League, not only its Administrator (unlike Settings/Invitations).

## UIR-209 — Remove Action, Administrator-Only, Never on Their Own Row
A Remove action is available only to the League Administrator, only on another member's row — never on the Administrator's own row, which uses the separate Leave action instead (UIR-210).

## UIR-210 — Leave League Action, Every Member's Own Row
Every member, including the Administrator, sees a Leave League action on their own row.

## UIR-211 — Sole-Administrator Leave Has No Resolution Path
Because the backend has no mechanism anywhere to transfer Administrator status (§12 item 13), a sole Administrator who clicks Leave League and receives the backend's resulting error sees a plain statement of that exact limitation — "You're this League's only Administrator, and there's currently no way to transfer that role, so you can't leave yet" — rather than a fabricated transfer control this document has no source material to design.

## UIR-212 — Removed/Left Members Stay Visible, Not Deleted
A member who has been removed or has left is not deleted from the list — their row remains, status changed to "Left," consistent with this product's established pattern of moving records to a distinct state rather than deleting them (UIR-033, UIR-140).

## UIR-213 — Members Is League-Scoped
Like every other League-scoped screen, Members reflects only the active League; switching Leagues re-scopes it entirely.
...
| Profile | ✓ | ✓ | ✓ |
| League Creation (added v1.5) | ✓ | ✓ | — |
| League Settings (added v1.5) | — | ✓ | — |
| Invitations (added v1.5) | — | ✓ | — |
| Members (added v1.5) | ✓ | ✓ | — |

*(Security and Username Display Policy resolved to System-Administrator-only, reached via the "Platform" nav section (UIR-006a) rather than the per-league "Admin" group — see v1.1's revisions to UIR-006, UIR-151, and UIR-156. League Creation has no League context to scope it to and is reachable by any authenticated user, including one belonging to zero Leagues — see UIR-194.)*
...
|---|---|
| Phase 1 — Foundation (Identity & Auth, User Profile & FantasyTeam, League & Season Management, EPL/FPL Data) | Profile, EPL, Login, Register, Forgot/Reset Password, Accept League Invitation (§8.17–8.20, added v1.4), League Creation, League Settings, Invitations, Members (§8.21–8.24, added v1.5) |
...
2. ~~**No league-creation or league-administration-setup screens exist.**~~ **Resolved in v1.5.** League Creation, League Settings, Invitations, and Members are now designed in §8.21–8.24, from the backend's already-implemented `createLeague`/`updateLeague`/`getLeagueConfiguration`/`updateLeagueConfiguration`/`createSeason`/`getSeasonConfiguration`/`updateSeasonConfiguration`/`createInvitation`/`listInvitations`/`revokeInvitation`/`leaveLeague` surface rather than from a mock-up (there still isn't one — see the note at the top of §8.21). Administrator transfer is explicitly not part of this resolution — see new item 13.
...
11. ~~**A Draft can be `Paused` by a League Administrator, with no corresponding UI anywhere.**~~ **Resolved in v1.6, for Draft Board only.** UIR-214–218 design the paused-state banner, blocked picks, replaced timer display, and the Administrator's pause/resume control. Unlike items 1–2's resolutions, this one had no `BR-###` rules to design from at all (the backend BRD has none), so the design is deliberately conservative where the contract is silent (see the note preceding UIR-214) rather than presenting a guess as confirmed behavior. Makeup Picks & Timeouts remains unbuilt (a separate, pre-existing gap) — applying this same treatment there is noted, not designed, pending that screen actually being built.
...
13. **Administrator transfer is referenced by the backend's own error response but implemented nowhere.** Discovered in v1.5 while designing League Members (§8.24): `leaveLeague`'s `409` response states "the sole League Administrator cannot leave without first transferring administration (`BR-025`, `BR-283`)" — but `BR-283` only states there is exactly one Administrator and rules out delegated co-administrator roles; it does not describe a transfer mechanism, and no endpoint anywhere in the API surface performs one (no request field, no separate operation, nothing). This is a genuine backend contract gap, not a UI one: the error message presupposes a capability that doesn't exist. Like item 11 (Draft-Paused), this is intentionally left undesigned here rather than invented without real source material — UIR-211 states the limitation plainly on the Members screen instead of fabricating a transfer control.
...
| DEC-UI-043 | (v1.5) This BRD's scope extends to design League Creation, League Settings, Invitations, and Members (§8.21–8.24), resolving §12 item 2 — except administrator transfer, which is recorded as a new open gap (item 13) rather than designed, since no backend mechanism for it exists anywhere. | The backend's League-management API surface has sat fully implemented and unconsumed since DEC-UI-007 deferred it in v1.1 (and DEC-UI-035 deferred it again in v1.4, deliberately, to keep that pass scoped to authentication only); administrator transfer specifically has no source material to design from — inventing one would mean guessing at a capability the backend doesn't offer. |
| DEC-UI-044 | (v1.5) League Settings exposes every parameter in `BR-291`'s configurable-parameter table, not only the six fields History's read-only config snapshot already surfaces. | Restricting editing to History's six would leave several backend-configurable parameters (secondary draft selections/scheduling offset, Gameweek roster lock offset, replacement selection cap) with no way to ever change them from their defaults — an arbitrary, unjustified restriction with no basis in `BR-290`'s own principle that these are all meant to be configurable. |
| DEC-UI-045 | (v1.5) Invitations never displays the invitation link or token itself, before or after sending. | The backend's `Invitation` schema carries no link/token field at all — delivery happens entirely server-side through the chosen channel (Email/SMS); there is nothing to display and no manual-share fallback to design around that absence. |
| DEC-UI-046 | (v1.5) League Members shows a Leave League action on the Administrator's own row too, even though a sole Administrator's attempt will always fail against the backend (item 13). | Consistency: every member, admin or not, manages their own membership the same way; the failure is explained in place (UIR-211) rather than avoided by hiding the action and leaving the Administrator to wonder whether leaving is possible at all. |
| DEC-UI-050 | (v1.6) Resolving §12 item 11 (Draft-Paused) is scoped to Draft Board only; Makeup Picks & Timeouts stays unbuilt and is not retrofitted with the same treatment in this pass. | The user's own scoping decision: Makeup Picks & Timeouts is a separate, pre-existing gap (it has no screen at all yet), and designing its paused-state behavior before the screen itself exists would mean designing in a vacuum. A forward-looking note is added at §8.10 instead, so the same treatment is applied deliberately once that screen is actually built, rather than rediscovering the same open questions independently. |
| DEC-UI-051 | (v1.6) Where the backend is silent on Draft-Paused behavior — no `pausedAt`/remaining-timer field, no documented pick rejection while paused — the client chooses the interpretation least likely to mislead: it blocks the Draft action client-side for everyone while paused (UIR-215), and replaces the timer display entirely with a static "Paused" indicator rather than showing any specific countdown value (UIR-216), instead of guessing at backend behavior and presenting the guess as confirmed. | This is a first for the project: even the auth (item 1) and league-administration (item 2) gaps had real `BR-###` rules to design from once found; Draft-Paused has none anywhere in the backend BRD. Rather than leave the gap undesigned indefinitely (as item 13, administrator transfer, correctly is, having no source material at all) or invent backend behavior that isn't documented, the client can still design its own defensive presentation — never trusting an undocumented backend to reject a paused pick, and never fabricating a countdown value that might not reflect reality. |
...
| v1.5 | 2026-09-11 | Adds §8.21–8.24 (UIR-191–UIR-213): League Creation, League Settings, Invitations, and Members — designed the same way §8.17–8.20 were, directly from the backend's already-implemented League-management surface with no mock-up behind any of them. Resolves §12 item 2 in full except administrator transfer, recorded as a new gap (item 13): the backend's own `leaveLeague` error references "transferring administration" but no endpoint or rule anywhere actually implements it. Updates §10's access matrix and §11's traceability table. Adds DEC-UI-043 through DEC-UI-046. |
| v1.6 | 2026-09-11 | Resolves §12 item 11 (Draft-Paused), scoped to Draft Board only by explicit choice, with Makeup Picks & Timeouts (still unbuilt) left for a future pass. Amends UIR-101 and UIR-103 to account for the paused state, and adds UIR-214–218: a paused-state explanation banner, blocking the Draft action for every manager while paused, replacing (not fabricating) the timer display, a League-Administrator-only pause/resume control, and propagation via existing polling with no manual refresh needed. Unlike every prior gap resolved in this document, the backend BRD has zero `BR-###` rules about pause/resume at all — the design is deliberately conservative wherever the contract is silent, and says so in the document itself rather than presenting an inference as confirmed behavior. Adds a forward-looking note at §8.10 so Makeup Picks & Timeouts applies the same treatment once that screen is built. No changes needed to §10 or §11 — no new screen or route is introduced. Adds DEC-UI-050 and DEC-UI-051. |
