...
- Authentication, registration, password reset, and league-invitation-acceptance screens — the mock-up starts from an already-authenticated session and does not depict them. They are required for a working product but are out of this BRD's scope until a v1.1 pass covers them explicitly (see §12).
...
- Visual design polish beyond what the mock-up already establishes (the mock-up's dark/light palette, typography, and component shapes are treated as an accepted starting point, not as final art direction).
- Any business rule not already reflected in a mock-up screen. If the backend BRD defines a rule with no corresponding screen (e.g., league creation, invitation sending), this document does not invent a screen for it — it lists the omission in §12 instead.
...
## UIR-006 — "Admin" Nav Group
A collapsible nav group labeled "Admin" contains: Audit Log, Score Corrections, Security, Username Display Policy. This group, and everything in it, must only render for a user who is a League Administrator for the active league **or** a System Administrator — see §12 for the Security-screen scoping question this raises.
...
## UIR-012 — Responsive Collapse of Navigation
Below a defined breakpoint, the primary navigation must remain fully reachable (the mock-up drops nav to its own wrapped row under the brand/league-switch/profile row) rather than being silently hidden. A mobile-specific navigation pattern (hamburger menu, bottom tab bar, etc.) is not mandated by the mock-up and is left to `02-architecture`.
...
Alongside the pitch, a panel lists the bench/reserve players not in this Gameweek's starting XI (`BR-038`), each with position, club, and next opponent; if the reserve list is long, it is truncated with a count and a link to the full Squad screen (UIR-009) rather than growing the panel indefinitely.
...
Because this screen is explicitly not league-scoped (UIR-145), reaching it must never require an active league to be selected, and it must render identically regardless of which league (if any) is currently active in the league switcher — see §12 for the navigation-placement inconsistency this creates in the current mock-up.
...
Unlike the Audit Log and Score Corrections, this policy is described as applying platform-wide to every historical record, not scoped to the active league — see §12 for the resulting question about why it is nonetheless reached via the same per-league "Admin" nav group as those two screens.
...
| Username Display Policy | — | *see §12* | *see §12* |
| Profile | ✓ | ✓ | ✓ |
...
|---|---|
| Phase 1 — Foundation (Identity & Auth, User Profile & FantasyTeam, League & Season Management, EPL/FPL Data) | Profile, EPL, the not-yet-designed auth/onboarding screens (§12) |
...
1. **No authentication, registration, or invitation-acceptance screens exist in the mock-up.** The application is depicted entirely from an already-authenticated session. A production client needs login, (if self-registration is in scope) registration, password reset, and a flow for accepting a league invitation (`BR-027`–`BR-029`) — none of which this BRD can specify from the mock-up alone.
2. **No league-creation or league-administration-setup screens exist.** League Administrator actions like creating a league, sending invitations, or configuring the settings later shown read-only on the History screen's "config snapshot" (squad size, positional minimums, draft timer, points-per-result, invitation expiration) have no corresponding input screen in the mock-up.
3. **The Security screen's navigation placement contradicts its own stated scope.** UIR-145/UIR-151 require the screen to be reachable without any league context, yet the mock-up places it inside the same per-league "Admin" nav group as Audit Log and Score Corrections (both genuinely league-scoped). Either the nav needs a structurally separate, non-league-scoped entry point for System-Administrator screens (recommended), or the platform's access model needs to clarify that a System Administrator is expected to also be viewing through some league context when they reach it. This should be resolved explicitly, not left to implementation convention.
4. **Username Display Policy has the same placement question, for a different reason.** UIR-156 notes this is a platform-wide, already-resolved setting — not per-league configuration — yet it also sits in the per-league Admin nav group. Unlike Security, it may be intentional (it's read-only/informational, so any administrator viewing it in any league context sees the same platform-wide answer) — but this document treats the two placements as the same open question rather than assuming the same or different resolution for each.
5. **No error, empty, or loading states are depicted anywhere in the mock-up.** Every screen assumes a happy-path, fully-populated data set. This BRD cannot specify what a manager sees on, e.g., a network failure during Submit Roster, a league with zero messages yet, or a Draft Board pool still loading — these need their own requirements pass.
6. **No pagination is shown anywhere**, including on screens (Audit Log, Messages, History) whose real data volume will eventually exceed a single unpaginated page. The mock-up's filtering (UIR-021) reduces *visible* rows but does not address *loaded* row volume at scale.
7. **The mechanism behind "live" data (in-progress EPL fixtures, live draft clocks) is unspecified.** Whether the client polls, holds a persistent connection, or something else is an architecture decision this BRD intentionally defers (see §9), but it is called out here so it isn't lost.
8. **Mobile-specific interaction patterns are unconfirmed.** The mock-up's responsive behavior (UIR-012) reflows grid columns and wraps the nav row, but doesn't depict a mobile-specific navigation pattern (e.g., a hamburger menu or bottom tab bar) — whether the reflow-only approach is sufficient for a genuinely mobile-first experience, versus needing a dedicated mobile pattern, is open.
