...
| Draft Board | `GET /drafts/{id}`, `POST /drafts/{id}/picks`, `GET /drafts/{id}/player-pool`, `GET /drafts/{id}/selections`, `POST /drafts/{id}/pause`, `POST /drafts/{id}/resume` (v1.5, League-Administrator-only) |
...
**v1.5 note (BRD UIR-218):** the existing 3–5 second Draft turn/timer poll is also what surfaces a `Paused`/`InProgress` transition to every viewer without a manual refresh — no separate poll or interval is introduced for this. While `status === 'Paused'`, the client stops rendering `<CountdownClock>` against `currentPickDeadline` entirely (§9's table below) rather than freezing it or letting it run, since the backend's `Draft` schema has no field recording what the remaining time was at the moment of pause — showing any specific value here would be a client-side guess, not a confirmed fact (see ADR-017).
...
| ADR-016 | (v1.5) `pauseDraft`/`resumeDraft` are gated the same way as Audit Log/Score Corrections/League Settings — a League-Administrator-only control, rendered directly on Draft Board rather than as a separate screen or route. | §6.4/BRD UIR-217 — both operations require no request body and return the full `Draft` object; there is no new resource or navigation destination here, just one more Administrator-only action alongside a screen that already exists, matching the existing access-gating pattern instead of inventing a new one. |
| ADR-017 | (v1.5) While `Draft.status === 'Paused'`, the client (a) disables the Draft action for every manager, on-clock or not, and (b) replaces `<CountdownClock>` with a static "Paused" indicator — both enforced/rendered client-side regardless of what the backend actually does, because the backend documents neither. | §8's v1.5 note/BRD UIR-215–216 — `pauseDraft`/`resumeDraft` and `makeDraftPick` have zero documented interaction with each other (no pause-aware error code, no `pausedAt`/remaining-timer field on `Draft`), a first for this project even among prior gaps that at least had `BR-###` rules to anchor to. Never trusting an undocumented backend to reject a paused pick, and never fabricating a countdown value the backend can't confirm, is the conservative choice BRD DEC-UI-051 calls for. |
...
| v1.5 | 2026-09-11 | Against BRD v1.6, which resolves §12 item 11 (Draft-Paused) for Draft Board only. §6.4's Draft Board row gains `pauseDraft`/`resumeDraft`. §8 gains a note confirming the existing 3–5 second turn/timer poll (not a new interval) is what surfaces the paused/resumed transition, and that the timer display is replaced entirely rather than frozen or left running while paused, since `Draft` carries no field recording remaining time at pause. No route or nav change — this stays entirely inside the existing Draft Board screen. Adds ADR-016 (League-Administrator-only pause/resume control, reusing the existing access-gating pattern) and ADR-017 (client-side-only enforcement of blocked picks and the replaced timer display, since the backend documents neither behavior). |
